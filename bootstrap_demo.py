...
def generate_plots():
    ...
    plt.axis('off')
    plt.savefig('dataset_architecture.png')
    plt.close()
    ...
    plt.savefig('batch_audit.png')
    plt.close()

    # 5. Diagnostics
    plt.figure(figsize=(10, 5))
    sns.kdeplot(np.random.randn(100), label='Legit')
    sns.kdeplot(np.random.randn(100) + 2, label='Fraud')
    plt.title('Feature Separation Analysis')
    plt.savefig('transaction_diagnostics.png')
    plt.close()
...
def bootstrap():
    ...
    # 2. Create Dummy Data
    if not os.path.exists("fraud_detection/data/creditcard.csv"):
    ...
    df = pd.read_csv("fraud_detection/data/creditcard.csv")
